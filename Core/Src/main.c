...
QueueHandle_t q_data_input;
QueueHandle_t q_data_print;

TimerHandle_t handle_led_timers[3];
TimerHandle_t rtc_timer;

volatile uint8_t user_data;

state_t current_state = state_MainMenu;
/* USER CODE END PV */
...
void rtc_report_callback(TimerHandle_t);
/* USER CODE END PFP */
...
int main(void)
{
  ...
  // If the control comes here, then the scheduler failed to start due to
